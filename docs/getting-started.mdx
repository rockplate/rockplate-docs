...
See [Advanced Usage](/advanced-usage#builder-and-parser) section to learn how to use the `Builder` and `Parser` classes instead of `Rockplate` class.

## Usage with an `.rpl` file

NOTE: If you're using Visual Studio Code, install the [Official Extension](https://marketplace.visualstudio.com/items?itemName=safrazik.rockplate-vscode) to get IntelliSense and Linting support for `.rpl` files.
...
You can embed schema in an `.rpl` file in json format on top of the file.
